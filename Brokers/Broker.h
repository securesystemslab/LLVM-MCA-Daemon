...
struct Broker {
  ...
  // requested by the caller. When it's -1 the Broker will give all MCInsts
  // it has. Of course, it's totally possible that Broker will only give out
  ...
  // index 0.
  // Return false if the there is no MCInst left
  virtual bool fetch(MutableArrayRef<const MCInst*> MCIS, int Size = -1) {
    return false;
  }
};
} // end namespace mcad
