...
TimelineView::TimelineView(const MCSubtargetInfo &sti, MCInstPrinter &Printer,
                           mcad::MetadataRegistry *MDR, llvm::raw_ostream &OS,
                           llvm::function_ref<GetInstCallbackTy> CB)
    : InstructionView(sti, Printer, CB), MDRegistry(MDR), OutStream(OS),
      CurrentCycle(0) {}
...
void TimelineView::onEvent(const HWInstructionEvent &Event) {
  ...
  if (auto MDTok = Inst.getIdentifier()) {
    mcad::MetadataRegistry &MDR = *MDRegistry;
    const auto &MarkerCat = MDR[mcad::MD_BinaryRegionMarkers];
...
void TimelineView::printTimeline(unsigned First, unsigned Last) const {
  ...
  unsigned TotalCycles = CurrentCycle - static_cast<unsigned>(FirstCycle);
